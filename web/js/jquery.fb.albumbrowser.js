                            ...
                            for (a = 0; a < $(result.data).length; a++) {
                                ...
                                if ($(result.data).get(a).count == null && settings.skipEmptyAlbums) {
                                    ...
                                    $.ajax({
                                        ...
                                        success: function (cover) {
                                            ...
                                            if (!$(cover).get(0).error && $(cover).get(0).images) {
                                                ...
                                                var albumThumb = $("<img>", { style: "display:none;", "data-id": $(cover).get(0).id, class: "fb-album-thumb", "data-src": albumImg.source, src: "" })
                                    ...
                                    $(albumListItem).hover(
                                        ...
                                        function() {
                                            var self = $(this),
                                                heightSelf = self.height(),
                                                heightChild = self.find("div.fb-album-title").height(),
                                                padding = (heightSelf-heightChild)/2;
                                            if (self.find("div.fb-album-title").css('padding-top') == '0px') {
                                                self.find("div.fb-album-title").css({
                                                    height : heightSelf+'px',
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        if (result.data.length > 0) {
                            ...
                            // $(commentsMore).css("width", container.find(".fb-preview-img").width() - 12);
                            $(commentsMore).css("width", container.find(".fb-preview-img").width());
                            ...
                            for (c = 0; c < result.data.length; c++) {
                                ...
                                // comment.css("width", container.find(".fb-preview-img").width() - 12);
                                comment.css("width", container.find(".fb-preview-img").width());
                ...
                $(photoLink).click(function (event) {
                    ...
                    if (settings.lightbox) {
                        ...
                        previewImage.load(function () {
                            ...
                            // previewText.css("width", $(this).width() - 12);
                            previewText.css("width", $(this).width());
                            // previewText.css("minWidth", $(this).width() - 12);
                            previewText.css("minWidth", $(this).width());
                            // $(".fb-comment,.fb-comment-more").css("width", $(this).width() - 12);
                            $(".fb-comment,.fb-comment-more").css("width", $(this).width());
                            $(this).show();
